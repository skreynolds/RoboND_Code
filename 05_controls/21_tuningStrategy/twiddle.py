# -*- coding: utf-8 -*-
"""
Created on Sun Nov  5 14:38:23 2017
...
def twiddle(tol=0.05):
    ...
    while sum(dp) > tol:
        ...
        i = 0 # Initialise i for a single loop
        
        # For loop runs through 0,1,2
        for i in range(0,3):
            # First implementation of error check
            p[i] += dp[i]
            soln = hover(p)
            cost = cost_fun(soln)
            
            # Second conditional block checking if improvement on cost fcn
            if cost < best_cost:
                # Response for improved cost
                best_cost = cost
                dp[i] *= 1.1
            else:
                p[i] -= 2*dp[i]
                ...
                # Final conditional block checking if improvement in cost fcn
                if cost < best_cost:
                    # Response for improved cost
                    best_cost = cost
                    dp[i] *= 1.1
