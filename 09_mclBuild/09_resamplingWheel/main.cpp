...
int main()
{
    ...
    // Resampling
    double beta = 0; //initialise beta as zero
    double max_w = max(w, n); //determine the max weight (modified w)

    int index = (int)n*gen_real_random();

    for (int i=0; i<n; i++){
    	// update beta
    	beta += (2*max_w)*gen_real_random();
    	// resample
    	while (w[index] < beta){
    		beta -= w[index];
    		index = mod((index + 1), n); //cycling round a wheel need mod
    	}
    	// replace the resampled particle
    	p[i] = p[index];
    	cout << p[i].show_pose() << endl;
    }
