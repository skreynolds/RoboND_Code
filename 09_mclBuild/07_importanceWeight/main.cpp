...
public:
    ...
    double measurement_prob(vector<double> measurement)
    {
    	...
    	// This function can access all of the variables attached to the particle object
    	// and is given an arguement z (which is the measurement from the actual robot)
    	// This is an important feature of the Object Oriented programming - being able
    	// to access variables which are "part of the object".

    	// Initialises variables for calculation
        double prob = 1.0;
        double dist;
        // Landmarks are a global variable; x, y, and sense_noise are part of the object;
        // measurement is a 8 element vector which is based on the actual robot location
        for (int i = 0; i < sizeof(landmarks) / sizeof(landmarks[0]); i++) {
            dist = sqrt(pow((x - landmarks[i][0]), 2) + pow((y - landmarks[i][1]), 2));
